...
*/
(function ($) {
    $.fn.jAutoCalc = function jAutoCalc(method) {
        var opts = {};

        var funcs = {
            'sum': {
                rgx: 'sum\\({([^}]+)}\\)',
                exec: function (field, ctx, numberFormat) {
                    m = 0;
        ...
        /*
            Takes a string contain an equation.  Returns an array containing all the field names used in the equation
            or empty array if it can't find any
        */
        var findFields = function findFields(eq) {
            fields = [];
            r = /{([^}]+)}/gi;
        ...
        var getFieldSelector = function getFieldSelector(field) {
        ...
        /*
            Takes the value from a field and strips it down to a numeric value, formatted with a period for the decimal.
            If an object is passed in, numberFormat information is stored like decimal symbol, thousand symbol,
            currency symbol and position for potential use later.
        */
        var numCleanse = function numCleanse(fieldValue, numberFormat) {
            numValue = '';
        ...
        */
        var doCalc = function doCalc(eq, fields, result, ctx) {
            field = '';
            fieldValue = '';
            numValue = '';
            resultvalue = '';
            ...
            if (resultValue == null) {
                ...
                resultValue = numberFix(resultValue, numberFormat.decPlaces);
            }
        ...
        */
        var numberFix = function numberFix(num, decPlaces) {
        ...
        */
        ...
        var parseVal = function parseVal(r) {
            ...
            if (r.offset > startOffset) { // did that work?
                ...
                throw 'parseError';
            } else if ((m = /^[a-z_][a-z0-9_]*/i.exec(r.string.substr(r.offset))) != null) { // variable/constant name
        ...
        var parseOp = function parseOp(r) {
        ...
        var parseExpr = function parseExpr(r) {
            var stack = [{
                precedence: 0,
                assoc: 'L'
            }];
            ...
            return this.each(function () {
                ...
                $('[' + opts.attribute + ']:not([_jac])', $ctx).each(function () {
                    ...
                    if (opts.keyEventsFire) {
                        fireEvents += ' keyup.jautocalc keydown.jautocalc keypress.jautocalc';
                    }
                    for (i = 0; i < fields.length; i++) {
                        field = fields[i];
        ...
        */
        ...
        if (publicMethods[method]) {
            ...
            return publicMethods.init.apply(this);
        }
    };
